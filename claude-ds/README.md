...
> 本目录是**独立工具 `claude-ds`**。仓库里另一个工具 `claude-go`（OpenCode Go 后端）见 [../claude-go](../claude-go)，两者互不依赖，可单独安装。
...
三者共用同一份 API Key：`~/.config/deepseek/api_key`。
...
├── README.md                     # 本文件
├── install.sh                    # 一键安装
├── claude-ds                     # 命令入口（被软链到 ~/.local/bin/）
├── deepseek_usage.sh             # 余额查询 CLI
├── DeepSeekUsage/                # 菜单栏 App
│   ├── DeepSeekUsage.swift       # 源码
│   ├── deepseek.svg              # 图标素材
│   ├── build.sh                  # 构建脚本
│   └── DeepSeekUsage.app         # 构建产物（不入库）
└── vendor/
    └── deepclaude/               # 上游开源项目（install.sh 克隆，不入库）
        └── deepclaude.sh
```
...
- **改菜单栏 App 图标**：替换 `DeepSeekUsage/deepseek.svg` 后在本目录执行 `cd DeepSeekUsage && ./build.sh`
...
- **v3.0**：与 `claude-go` 拆分为两个独立目录，各自独立安装
- v2.1：曾内置 `claude-go`（现已拆分至 [../claude-go](../claude-go)）
